...
export class HttpClientAxios extends HttpClient {
  ...
  request<O = AxiosRequestConfig, R = AxiosResponse<any, any>>(
    options: O
  ): Observable<R> {
    return defer(() =>
      from(HttpClientAxios.instance.request(options as AxiosRequestConfig))
    ) as Observable<R>;
  }

  requestWithAuth<O = AxiosRequestConfig, R = AxiosResponse<any, any>>(
    options: O
  ): Observable<R> {
